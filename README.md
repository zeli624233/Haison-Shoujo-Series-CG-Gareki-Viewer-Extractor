...
<pre>
> 说明：
> 1.废村1的“IF线”内容，解包发现没有，没做测试，不一定支持。
> 2.废村2解包发现：st目录中：面部红晕差分图会有白底问题，无法用于立绘合成。
>   如果你对这方面有需求，可以看看EscudeTools的fork项目。
> 3.「废村1：籠愛拾遺」解包发现：ev目录下，同样有面部红晕差分图有白底问题，解决方法同上。
</pre>
## 额外（基本）支持
- 1.悠刻のファムファタル
...
<pre>
> 说明：
> 1.发现悠刻，戦巫，姫と艶欲的解包，同样有面部红晕差分图有白底问题，解决方法同上。
> 2.发现悠刻，戦巫，姫と艶欲的解包，st目录差不多也有红晕差分图有白底问题，解决方法同上。
> 3.这里的游戏我玩的不多，我只能保证：软件对bg和ev目录的识别和准确率基本没问题。
